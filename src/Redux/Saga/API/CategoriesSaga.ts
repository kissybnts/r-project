import { CategoryFetchRequestAction, isCategoryFetchRequestAction } from '../../../App/Language/Sentences/Actions';
import { call, put, take } from 'redux-saga/effects';
import { APIResponses } from '../../../API/Common';
import { CategoriesAPI } from '../../../API/Categories/API';
import { createCategoryFetchRequestSuccessAction } from '../../../App/Language/Sentences/ActionCreators';
...
export function* handleFetchCategory() {
  while (true) {
    const action: CategoryFetchRequestAction = yield take(isCategoryFetchRequestAction);
    const apiResponses: APIResponses = yield call(CategoriesAPI.fetchCategory, action.id);
    if (!apiResponses.error) {
      yield put(createCategoryFetchRequestSuccessAction(<CategoryDetailResponse> apiResponses.response.data));
    } else {
      console.log(apiResponses.error);
    }
  }
